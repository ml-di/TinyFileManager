...
import ru.sitnikovdi.tinyfilemanager.Util.StorageMemory;
...
public class MainActivityPresenter extends BasePresenter<MainActivityViewInterface> implements MainActivityPresenterInterface {
    ...
    public ArrayList<Parcelable> getStorageArrayList() {
        ...
        // Внутренняя память
        final long internalFreeMemory = StorageMemory.getFreeSpace(0, getView().getContext());
        final long internalTotalMemory = StorageMemory.getTotalSpace(0, getView().getContext());

        storageList.add(new RecyclerViewStorageMainData(0, internalTotalMemory, internalFreeMemory, R.drawable.ic_phone_24px));

        // MicroSD карта
        if (StorageMemory.isSDCardAvailable(getView().getContext())) {
            storageList.add(new RecyclerViewStorageMainData(
                    1,
                    StorageMemory.getTotalSpace(1, getView().getContext()),
                    StorageMemory.getFreeSpace(1, getView().getContext()),
